...
/**
 * Trims each holding's returns to entries on or before the requested end date — the user's {@code cped} when supplied,
 * otherwise the snapshot's inferred performance end date. Produces a new snapshot with a freshly built returns map and
 * a refreshed performance window so downstream consumers do not observe a stale {@code performanceEndDate}; the
...
public class CutByCpedOrPedProcessor implements ReturnsProcessor {
  ...
  public <T extends ReturnsData> ReturnsSnapshot<T> process(ReturnsSnapshot<T> snapshot, ProcessingContext context) {
    LocalDate effectiveEndDate = context.cped() != null ? context.cped() : snapshot.performanceEndDate();
