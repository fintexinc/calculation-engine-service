...
import static ca.tangerine.pce.application.util.SecurityDataValidator.requireDataForEveryHolding;
...
import static ca.tangerine.pce.model.error.ErrorCode.HOLDING_TYPE_NOT_LEAF;
...
public abstract class AbstractFeeCalculationService<R extends BaseCalculationResult>
   ...
   * {@link ErrorCode#NO_SECURITY_DATA_FOR_HOLDING} immediately — we won't silently treat an unknown fund as 0% fee,
   * because that would under-report the MER and Fees results without the caller noticing.
   *
   * <p>
   ...
   * fee calculator doesn't bucket — i.e. anything not in {@link HoldingTypeGroup#MER_BEARING_TYPES} ∪
   * {@link HoldingTypeGroup#ZERO_MER_TYPES}. Without this guard, an unrecognised leaf type would fall through the
   * resolver with {@code modifiedFee = null} and be silently dropped from both numerator and denominator in
   * {@link FeeAggregationMode#WHOLE_PORTFOLIO}, producing a wrong result instead of a clear error.
   */
