 ...
 * the formula cannot resolve is dropped from that holding's series, and a single {@link ErrorCode#FX_RATES_UNAVAILABLE}
 * {@link Notification} is appended to the warnings list per affected holding. The caller surfaces those warnings on the
 * response; the calculation continues with the partial converted series.
 */
@Slf4j
@Service
...
public class FxRateService {
   ...
   * {@link ErrorCode#FX_RATES_UNAVAILABLE} {@link Notification} is appended to {@code warnings} per affected holding
   * (regardless of how many month-ends were dropped). Holdings already in {@code toCurrency} pass through unchanged.
   */
   ...
   * {@link ErrorCode#FX_RATES_UNAVAILABLE} warning, matching the behaviour of {@link #convertReturns}.
   */
