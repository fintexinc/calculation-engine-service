...
/**
 * End-to-end coverage for the {@code /equity-country-exposure} endpoint. Security Master answers with the countries a
 * security's equity sleeve is invested in; the metric rolls those onto the five {@link CountryRegionType} buckets
 * through the {@code country-allocation-mapping.json} table and reports one distribution for the portfolio.
 *
 * <p>
 ...
 * Security Master has no country data for: its weight leaves the reported distribution entirely and the remaining
 * buckets are rescaled over the hole, which only a warning tells the client about.
 *
 * <p>
...
class EquityCountryExposureE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForSmsUnavailableScenario() {
    return writeJson(exposureCommand(fundCa(NORTH_AMERICAN_FUND, 50_000), etfCa(WORLD_ETF, 50_000)));
  }
   ...
   *
   * <p>
   * Individual stocks are deliberately absent. Security Master serves the country breakdown for composite securities
   * only, so a stock arrives with no data and is reported as a gap — the subject of
   * {@link #shouldWarnAndRescaleOverTheHole_whenSecurityMasterHasNoCountryAllocation()} rather than of the positive
   * case.
   */
  @Override
  protected String requestBodyForPositiveSmsScenario() {
    return writeJson(exposureCommand(
        fundCa(NORTH_AMERICAN_FUND, 40_000),
        fundCa(DEVELOPED_FUND, 20_000),
  ...
  protected String requestBodyForMismatchedMetricScenario() {
    ...
    command.setHoldings(List.of(fundCa(NORTH_AMERICAN_FUND, 50_000)));
  ...
  /**
   * The two shapes a data gap arrives in — a security Security Master has no row for at all, and a row it serves with
   * no allocations on it — are the same gap to the client, and both have to be reported. What the reported numbers then
   * mean is the point of the assertion: the missing holdings' weight does not go to a bucket of its own (this taxonomy
   * has no UNKNOWN), it leaves the distribution, and what remains is rescaled to 100%. The fund that does carry data
   * holds half the portfolio and comes back describing all of it.
   */
  @Test
  void shouldWarnAndRescaleOverTheHole_whenSecurityMasterHasNoCountryAllocation() {
    PortfolioHolding allocated = fundCa(NORTH_AMERICAN_FUND, 50_000);
    PortfolioHolding withoutAllocations = fundCa(DEVELOPED_FUND, 30_000);
    PortfolioHolding unknownToSecurityMaster = etfCa(WORLD_ETF, 20_000);
    smsMockServer.setDispatcher(
    ...
    var response = postCalculation(writeJson(
        exposureCommand(allocated, withoutAllocations, unknownToSecurityMaster)));
    ...
    assertMissingExposureWarnings(result.getWarnings(), withoutAllocations, unknownToSecurityMaster);
    assertDistribution(result.getEquityCountryExposure(), CountryRegionType.class, Map.of(
  ...
  void shouldReportEveryBucketAsNull_whenNoHoldingHasCountryAllocation() {
    PortfolioHolding firstUnknown = fundCa(NORTH_AMERICAN_FUND, 50_000);
    PortfolioHolding secondUnknown = etfCa(WORLD_ETF, 50_000);
    smsMockServer.setDispatcher(
        singleAttributeDispatcher(CompositeSecurityAttribute.EQUITY_COUNTRY_ALLOCATION, List.of()));
  ...
  /**
   * The row Security Master serves for a security that declares the datapoint but has no countries on it — the
   * datapoint is assembled from whichever of its columns the security's table carries, so an allocation-less row is a
   * shape the client actually meets, distinct from no row at all.
   */
