...
class CommonPerformanceDatesE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForSmsUnavailableScenario() {
    return writeJson(commonDatesCommand(
        List.of(List.of(etfCa(EARLY_START_ETF, 50_000)), List.of(etfCa(LATE_START_ETF, 50_000))),
        List.of(etfCa(BENCHMARK_ETF, 100_000))));
  }
  ...
  protected String requestBodyForPositiveSmsScenario() {
    return requestBodyForSmsUnavailableScenario();
  }
   ...
   * than one. A dispatcher answers both from the same set of rows, which is what Security Master would do: the rows a
   * caller receives are the ones it asked identifiers for.
   */
  @Override
  ...
  protected String requestBodyForMismatchedMetricScenario() {
    ...
    command.setHoldings(List.of(etfCa(EARLY_START_ETF, 50_000)));
  ...
  void shouldDropTheHoldingAndWarn_whenTheHistoriesDoNotOverlap() {
    PortfolioHolding older = etfCa(EARLY_START_ETF, 50_000);
    PortfolioHolding newer = etfCa(LATE_START_ETF, 50_000);
    smsMockServer.setDispatcher(
    ...
    assertThat(warning.getMessage()).isEqualTo(ErrorCode.HOLDING_PSD_OUT_OF_RANGE.getMessage());
    ...
    assertThat(warning.getMetadata())
        .as("the message names no holding, so the metadata is the only place the caller can learn which one was dropped")
        .containsEntry("holdingId", newer.getIdsString());
