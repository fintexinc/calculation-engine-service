 ...
 * {@code COUNTRY_ALLOCATION} datapoint — the one issuer-country breakdown Security Master publishes per security — and
 * rolls it onto the five {@link CountryRegionType} buckets, so what this test pins over the wire is the taxonomy the
 * bond world needs and the equity one does not: an exposure with no country at all.
 *
 * <p>
...
class FixedIncomeCountryExposureE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForSmsUnavailableScenario() {
    return writeJson(exposureCommand(fundCa(CANADIAN_BOND_FUND, 50_000), etfCa(BOND_ETF, 50_000)));
  }
  ...
  protected String requestBodyForPositiveSmsScenario() {
    return writeJson(exposureCommand(
        ...
        etfCa(BOND_ETF, 30_000),
        fundCa(GLOBAL_BOND_FUND, 10_000),
   ...
   * a gap, because Security Master publishes the country breakdown for composite securities only. The client therefore
   * gets a warning naming the stock rather than a silently smaller pie, and the fund that does carry data describes the
   * whole of the reported distribution.
   */
  @Test
  void shouldWarnAndRescaleOverTheHole_whenTheHoldingHasNoCountryAllocation() {
    PortfolioHolding bondFund = fund(CANADIAN_BOND_FUND, FinancialInstrumentType.MUTUAL_FUND, Country.CANADA, 60_000);
    PortfolioHolding individualStock = stockCa("RY.TO", "TSX", 40_000);
    smsMockServer.setDispatcher(
