...
/**
 * Shared helpers for stubbing the Security Master {@code /attributes} endpoint in bootstrap-level e2e tests, and for
 * assembling the attribute rows it answers with. Every breakdown metric fetches its data through that one endpoint, so
 * the dispatcher, the row envelope and the datapoint wrappers are the same in every such test; only which attributes
 * are asked for and what they carry is metric-specific.
 *
 * <p>
 ...
 */
@UtilityClass
final class SmsAttributeResponses {
  ...
  /**
   * Answers a metric that asks for several attributes at once: Security Master serves those from the collection
   * endpoint, keyed by attribute.
   */
  ...
  /**
   * Answers a metric that asks for exactly one attribute, which Security Master serves from
  ...
  /**
   * One attribute row as Security Master serves it: the identifier the holding was fetched by, paired with the
   * attribute payload. Built from the identifier's parts rather than a {@link SecurityIdentifier} so a fixture reads as
   * the ticker or Morningstar id it is about.
   */
   ...
   * its region from {@code businessCountry} when Security Master knows it and falls back to the coarse
  ...
  /**
   * The row Security Master serves for a security that declares the datapoint without any regions on it — the datapoint
