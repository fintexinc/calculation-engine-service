...
public abstract class PropertiesHolder {
  ...
  /**
   * Get the ENVIRONMENT to execute the test test from the system property `EnvironmentProfiles` If no values are
   * passed, then the Default Test ENVIRONMENT will be used.
   *
   * @return EnvironmentProfiles
   * @throws IllegalArgumentException
   *           - if an invalid value of type EnvironmentProfiles is passed
   */
  private static EnvironmentProfiles getTestEnvironment() {
    ...
    if (environment == null) {
      ...
      Logger.warn("`{}` value is not passed in the command line, running the test in the default ENVIRONMENT -`{}`",
          profile, testEnvironment.getDescription());
      Logger.info(highlighterLine);
    ...
    if (testEnvironment == null) {
      throw new IllegalArgumentException("The value `" + environment + "` passed for the `" + profile
          + "` system property is invalid," +
          "check the enum " + EnvironmentProfiles.class.getTypeName());
    }
    Logger.info("Test Environment set to - `{}` which is `{}`", testEnvironment.getName(), testEnvironment
        .getDescription());
    Logger.info(highlighterLine);
    return testEnvironment;
  }
