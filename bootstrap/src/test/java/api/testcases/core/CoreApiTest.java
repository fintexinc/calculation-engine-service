...
/**
 * @param <T>
 *          stands for: Test Case Type
 * @param <H>
 *          stands for: Holding Type
 * @param <R>
 *          stands for: Application Response Object
 * @param <E>
 *          stands for: Expected & Actual results type
 */
...
public abstract class CoreApiTest<T extends CoreTestCaseModel, H extends HoldingDataDTO, R, E> {
  ...
  protected void closeResources() {
    try {
      ...
      String message = String.format("You had en error while closing workbook %s, error message %s",
          this.excelRelativePath, e.getMessage());
      log.error(message);
    }
  }

  /**
   * For debugging purposes only! If doesn't work then try to override existed excel.
   *
   * @param workbookName
   *          just the filename without extension, example: test_excel_name
   */
  protected void saveWorkbook(String workbookName) {
    try {
      ...
      String message = String.format("You had en error while saving workbook %s, error message %s",
          this.excelRelativePath, e.getMessage());
      log.error(message);
    }
  }
  ...
  private Arguments buildArguments(Map.Entry<Integer, T> entry, Supplier<Results<E>> results, String dataPointName) {
    ...
    String description = dataPointName.replace("-", " ").trim() + " " + this.testCases.get(entry.getKey())
        .getDescription().trim();
   ...
   *
   * @param tabs
   *          tabs which have to be populated before new test case
   * @param master
   *          tab that contains master expected results
   * @param testCaseEntry
   *          KEY - test case number, VALUE - test case body
   * @return test case executor
   */
  private Supplier<Results<E>> buildDelayedTestCaseExecutor(List<WritableSpreadsheet> tabs, Sheet master,
      Map.Entry<Integer, T> testCaseEntry) {
    return () -> {
      ...
      if (TestProperties.getCEVersions().isEnableCE2x0()) {
        ...
        log.info("getting actual results, testCase: {}, took {}ms", testCaseNumber, System.currentTimeMillis()
            - start4);
        log.info("In total testCase: {}, took: {} seconds", testCaseNumber, (System.currentTimeMillis() - start1)
            / 1000);
        return actualResult;
      }
  ...
  private void validateOtherCEVersions(ExpectedResultWrapper<E> expected, Map.Entry<Integer, T> testCaseEntry) {
    for (CoreAdaptor<T, H, R, E> adaptor : adaptors) {
      if (CEVersion.CE_1x5.equals(adaptor.getCEVersion())
        ...
        if (CEVersion.CE_1x3.equals(adaptor.getCEVersion())
   ...
   * @param testCaseEntry
   *          KEY - test case number, VALUE - test case body
   * @param expectedResultWrapper
   *          results from EXCEL
   * @return actual and expected results
   */
  private Results<E> queryApplication(Map.Entry<Integer, T> testCaseEntry,
      ExpectedResultWrapper<E> expectedResultWrapper) {
    ...
    if (expectedResultWrapper.is2xxSuccessful() && Objects.isNull(expectedResultWrapper.getOnFailedResponseFromApp())) {
      ...
      String appResStr = RestUtils.executeAppRequest(this.resourceURL, appRequestBody, expectedResultWrapper
          .getHttpStatus());
  ...
  protected void populateTabsBeforeEachTestCase(List<WritableSpreadsheet> tabs, Integer testNumber, T testCaseBody) {
    ...
    RequestParamsSupplier params = new RequestParamsSupplier(testCaseBody.getDataProviders(), testCaseBody
        .getCurrency());
  ...
  /**
   * @param weights
   *          that are part of some test case
   ...
   *
   * @param appResponse
   *          the whole application response
   ...
   *
   * @param master
   *          tab that contains expected data
