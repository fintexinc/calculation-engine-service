...
class AssetAllocationEmE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForMicUnavailableScenario() {
    return writeJson(allocationsCommand(CalculationMetric.ASSET_ALLOCATIONS_EM,
        fund(BALANCED_FUND, 50_000), etf(EMERGING_MARKETS_ETF, 50_000)));
  }
  ...
  protected String requestBodyForMismatchedMetricScenario() {
    ...
    command.setHoldings(List.of(fund(BALANCED_FUND, 50_000)));
   ...
   * same Market Investment Catalogue data sent to both endpoints, which must agree everywhere except emerging markets.
   * Here that is 28% of the portfolio — reported on its own by this metric, and added to international equities by
  ...
  private static PortfolioHolding[] positivePortfolio() {
    return new PortfolioHolding[] {
        fund(BALANCED_FUND, 40_000),
        etf(EMERGING_MARKETS_ETF, 20_000),
