...
class NumberOfUniqueHoldingsE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForMicUnavailableScenario() {
    return writeJson(uniqueHoldingsCommand(fund(CANADIAN_FUND, 50_000), fund(GLOBAL_FUND, 50_000)));
  }
  ...
  protected String requestBodyForPositiveMicScenario() {
    return writeJson(uniqueHoldingsCommand(fund(CANADIAN_FUND, 60_000), fund(GLOBAL_FUND, 40_000)));
  }
   ...
   *
   * <ul>
   * <li>a security Market Investment Catalogue has no record of — reported per holding, and counted as one holding of
   * its own;</li>
  ...
  void shouldCountUnresolvedSecuritiesAsOneEach_andWarnOnEachReason() {
    PortfolioHolding resolved = fund(CANADIAN_FUND, 25_000);
    PortfolioHolding unknownToMarketInvestmentCatalogue = fund(GLOBAL_FUND, 25_000);
    PortfolioHolding wrongIdentifierType = etf(WORLD_ETF, 25_000);
    PortfolioHolding nullIdentifierValue = fund("F00000UNQ3", 25_000);
    micMockServer.setDispatcher(
    ...
    var response = postCalculation(writeJson(uniqueHoldingsCommand(
        resolved, unknownToMarketInvestmentCatalogue, wrongIdentifierType, nullIdentifierValue)));
    ...
    assertSecurityNotFoundWarning(warningWith(result, ErrorCode.SECURITY_NOT_FOUND_FOR_METRIC),
        unknownToMarketInvestmentCatalogue);
  }
