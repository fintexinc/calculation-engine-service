...
class FeesE2ETest extends AbstractPortfolioCalculationE2ETest {
  ...
  protected String requestBodyForMicUnavailableScenario() {
    return writeJson(feesCommand(null, fund(CANADIAN_FUND, 50_000), etf(US_ETF, 50_000)));
  }
  ...
  void shouldRejectTheRequest_whenAFeeBearingHoldingHasNoCurrency() {
    PortfolioHolding fundWithoutCurrency = fund(CANADIAN_FUND, 300_000);
    micMockServer.setDispatcher(feesDispatcher(
    ...
    assertThat(notification.getCode()).isEqualTo(ErrorCode.Codes.HOLDING_MISSING_CURRENCY_FROM_MIC);
    assertThat(notification.getMessage())
        .isEqualTo(ErrorCode.HOLDING_MISSING_CURRENCY_FROM_MIC.getFormattedMessage(holdingId));
