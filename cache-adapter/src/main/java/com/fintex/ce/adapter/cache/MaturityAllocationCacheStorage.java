...
public class MaturityAllocationCacheStorage
  ...
  public Map<Holding, Map<MaturityAllocationType, BigDecimal>> load(final List<Holding> holdings,
    ...
    map.putAll(mapForNoneStock(loadCanadaHedgeFunds(filterHoldings(holdings, CANADA_HEDGE_FUND_PREDICATE), List.of()),
        warnings));
    map.putAll(mapForNoneStock(loadBenchOfFixedIncomes(filterHoldings(holdings, FIXED_INCOME_PREDICATE), List.of()),
        warnings));
    return map;
  }

  public <H extends Holding> Map<Holding, Map<MaturityAllocationType, BigDecimal>> mapForNoneStock(
      final Map<H, MaturityAllocation> holdings,
      final List<Warning> warnings) {
    return holdings.entrySet().stream().collect(toMap(Map.Entry::getKey, e -> maturityAllocationMapper(e, warnings)));
  }

  public <H extends Holding> Map<MaturityAllocationType, BigDecimal> maturityAllocationMapper(
      final Map.Entry<H, MaturityAllocation> entry,
      final List<Warning> warnings) {
