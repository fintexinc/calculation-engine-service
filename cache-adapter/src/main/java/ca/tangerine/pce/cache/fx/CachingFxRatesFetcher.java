 ...
 * <p>
 * The boundary inversion uses {@link BigDecimalConstants#INVERSE_SCALE} with {@link BigDecimalConstants#ROUNDING_MODE}
 * so cached and un-cached output stay bit-identical with adapters that fall back to inverse pairs internally.
 */
@Slf4j
...
public class CachingFxRatesFetcher implements FxRatesFetcher {
  ...
  /**
   * Inverts every rate in {@code rates} using {@link BigDecimalUtils#invert(BigDecimal)} so the non-canonical-direction
   * output of this caching wrapper matches what the un-cached delegate would have produced.
   */
