...
class CacheWarmUpServiceImplTest {
  ...
  void convertToSpecificHolding_stock() {
    ...
    final String holdingId = "FUND";
    final String exchangeId = "12";
    when(statistic.getHoldingId()).thenReturn(HoldingType.CANADA_STOCKS + DELIMITER + holdingId + DELIMITER
        + exchangeId);
  ...
  void reloadCache_verifyLoad() {
    ...
    // VERIFY
    verify(storage).load(List.of(h1, h2), List.of(), List.of(DataProvider.MORNINGSTAR), new ParamHolderDTO(
        calculateInitialPortfolioWeight(List.of(h1, h2))));
  }
