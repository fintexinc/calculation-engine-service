...
import static com.fintex.ce.adapter.rest.validation.chainofresponsibility.HoldingReqValidation.throwException;
...
class HoldingReqValidationTest {
  ...
  void validateHoldings_BENCHMARK_INDEX_holdingIdentifierIsNull() {
    ...
    final ReqValidationException expected = throwException(f,
        "Benchmark index holding identifier could only be specified as: " + HoldingIdentifierType.MORNINGSTAR_ID);
  ...
  void testValidateGicHoldingForValidData() {
    ...
    Method validateGicHolding = ReflectionUtils.findMethod(HoldingReqValidation.class, "validateGicHolding",
        Holding.class);
