...
public class NotNullCashCurrencyValidation extends ReqValidation {
  ...
  /**
   * Validates if currency value for Cash Holding type is not null. Note: this validation will be applied only if there
   * is only one Cash Holding in request payload, otherwise currencies can be validated using
   * {@link HoldingReqValidation} class.
   */
  @Override
