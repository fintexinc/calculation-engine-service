...
import com.fintex.ce.adapter.rest.observability.CalculationStatisticsReport.DurationStatistics;
import com.fintex.ce.adapter.rest.observability.CalculationStatisticsReport.MetricStatistics;
import com.fintex.ce.adapter.rest.observability.CalculationStatisticsReport.Overall;
import com.fintex.ce.adapter.rest.observability.CalculationStatisticsReport.WarningStatistics;
...
/**
 * Actuator endpoint that turns the per-metric calculation meters into a ranked summary, so the most problematic
 * calculation metrics can be seen without an external metrics backend. Exposed at {@code /actuator/calculationstats}
 * once the id is added to {@code management.endpoints.web.exposure.include}. Rows are ordered by absolute failure count
 * first and failure ratio second, so a metric that fails often on real traffic outranks one that failed its only call.
 */
@Component
...
public class CalculationStatisticsEndpoint {
  ...
  public CalculationStatisticsEndpoint(MeterRegistry meterRegistry) {
