...
class RollingTotalReturnsCalculationTest {
    ...
    void calculateRollingValue_checkResult() {
        ...
        final var sut = mock(RollingTotalReturnsCalculation.class, withSettings().useConstructor(calculationDTO, Set.of(), trailingTotalReturnsCalculation));
    ...
    void defineResponseType_checkResult() {
        ...
        final var expected = new RollingTotalReturnsResDTO(Set.of(resDTO));

        when(sut.getRollingIntervalResDTOS(anySet())).thenReturn(Set.of(resDTO));

        doCallRealMethod().when(sut).defineResponseType(result);
        //ACT
        final RollingTotalReturnsResDTO actual = sut.defineResponseType(result);
