...
class AnnualReturnCalculationTest {
    ...
    void populateBasicDetails_checkResult() {
        ...
        //ACT
        final AnnualReturnResDTO resDTO = new AnnualReturnResDTO(List.of(), List.of());
    ...
    void calculate_verifyPopulateBasicDetails() {
        ...
        final List<KeyValueDTO> keyValueDTOS = years.entrySet().stream().map(e -> new KeyValueDTO(e.getKey(), e.getValue())).collect(Collectors.toList());
    ...
    void calculate_checkResult() {
        ...
        doCallRealMethod().when(sut).calculate();
        //ACT
        final AnnualReturnResDTO actual = sut.calculate();
