...
class SharpeRatioCalculationTest {
    ...
    void defineResponseType_checkResult() {
        ...
        final TimeIntervalResDTO intervalResDto = new TimeIntervalResDTO("2000-01-12", ZERO);
        final TimeIntervalResDTO intervalResDto1 = new TimeIntervalResDTO("2020-01-05", BigDecimal.ONE);
        final Set<TimeIntervalResDTO> expected = Set.of(intervalResDto, intervalResDto1);
        when(sut.formTimeIntervalResDTO(anySet())).thenReturn(expected);

        doCallRealMethod().when(sut).defineResponseType(anySet());
        //ACT
        final SharpeRatioResDTO sharpeRatioResDTO = sut.defineResponseType(pairs);
    ...
    void getStandardDeviation_verifyCalculateExcessReturn() {
        try (var mockedPeriodCalculationAbstract = Mockito.mockStatic(PeriodCalculationAbstract.class)) {
            ...
            //VERIFY
            mockedPeriodCalculationAbstract.verify(() -> PeriodCalculationAbstract.calculateExcessReturn(returns, sut.tBills));
        }
    }
    ...
    void getStandardDeviation_verifyPeriodForNumberOfMonths() {
        try (var mockedPeriodCalculationAbstract = Mockito.mockStatic(PeriodCalculationAbstract.class)) {
            ...
            mockedPeriodCalculationAbstract.when(() -> PeriodCalculationAbstract.calculateExcessReturn(any(), any())).thenReturn((TreeMap) returns);
            when(sut.standardDeviationCalculation.calculatePeriodForNumberOfMonths(anyInt(), any())).thenReturn(ONE);
