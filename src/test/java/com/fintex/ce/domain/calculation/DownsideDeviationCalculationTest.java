...
class DownsideDeviationCalculationTest {
    ...
    void calculateDownsideReturnSquared_checkResult() {
        ...
        final var sut = mock(DownsideDeviationCalculation.class);
        final var date = LocalDate.now();
        final var portfolioExcessReturnsInPeriod = new TreeMap(Map.of(date, ONE, date.plusMonths(1), BigDecimal.valueOf(-5.2222), date.plusMonths(2), TEN_THOUSAND));
    ...
    void defineResponseType_checkResult() {
        ...
        final var intervalResDto1 = new TimeIntervalResDTO("2020-01-05", BigDecimal.ONE);
        ...
        //ACT
        final DownsideDeviationResDTO actual = (DownsideDeviationResDTO) sut.defineResponseType(pairs);
        //VERIFY
        Assertions.assertEquals(expected, actual.getDownsideDeviation());
    }
}