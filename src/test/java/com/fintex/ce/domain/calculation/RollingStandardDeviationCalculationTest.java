...
class RollingStandardDeviationCalculationTest {
    ...
    void calculateRollingValue_checkResult() {
        ...
        final var sut = mock(RollingStandardDeviationCalculation.class, withSettings().useConstructor(calculationDTO, Set.of(), standardDeviationCalculation));
    ...
    void defineResponseType_checkResult() {
        ...
        final var expected = new RollingStandardDeviationResDTO(Set.of(resDTO));

        when(sut.getRollingIntervalResDTOS(anySet())).thenReturn(Set.of(resDTO));

        doCallRealMethod().when(sut).defineResponseType(result);
        //ACT
        final RollingStandardDeviationResDTO actual = sut.defineResponseType(result);
