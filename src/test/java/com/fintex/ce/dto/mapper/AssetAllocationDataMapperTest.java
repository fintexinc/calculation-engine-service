...
class AssetAllocationDataMapperTest {
    ...
    void mapForAAEM_checkResult() {
        ...
        final Map<EtfHolding, RAssetAllocation> etfCanadaAssetAllocation = new HashMap<>();
        final var rAssetAllocationForEtfCanada = new RAssetAllocation().setHoldingType(HoldingType.CANADA_ETF)
        ...
        final Map<Holding, Pair<DataProvider, Map<AssetAllocationRegion, BigDecimal>>> expected = getExpected(etfUs, fundSeriesHolding, benchmarkIndexHolding,
        ...
        expected.put(cashHolding, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.CASH, BigDecimal.ONE))));
        expected.put(etfCanada, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.UNCLASSIFIED, BigDecimal.ONE))));
        expected.put(gicHolding, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.CASH, BigDecimal.ONE))));
    ...
    void mapForAA_checkResult() {
        ...
        final Map<Holding, Pair<DataProvider, Map<AssetAllocationRegion, BigDecimal>>> expected = getExpected(etfUs, fundSeriesHolding,
                benchmarkIndexHolding, canadaPooledFundHolding, canadaHedgeFundHolding, usMutualFundHolding, fixedIncomeHolding, smaHolding);
        expected.putAll(getExpectedWithSpecifiedDataProvider(null, stocksHoldings));
        expected.put(cashHolding, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.CASH, BigDecimal.ONE))));
        expected.put(etfCanada, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.UNCLASSIFIED, BigDecimal.ONE))));
        expected.put(gicHolding, new ImmutablePair<>(null, overrideDefaultValues(AssetAllocationDataMapper.DEFAULT_MAP, Map.of(AssetAllocationRegion.FIXED_INCOME, BigDecimal.ONE))));
        final var expectedWithProperFormat = expected.entrySet().stream().collect(toMap(Map.Entry::getKey, e -> e.getValue().getValue()));
    ...
    private HashMap<Holding, Pair<DataProvider, Map<AssetAllocationRegion, BigDecimal>>> getExpected(final Holding... holdings) {
    ...
    private <H extends Holding> Map<H, RAssetAllocation> getFdsResponse(final H holding) {
        final var result = new HashMap<H, RAssetAllocation>();
        ...
        final var rAssetAllocation = new RAssetAllocation();
