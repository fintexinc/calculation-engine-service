...
class MonthlyReturnsCacheStorageTest {
    ...
    void addCashReturns_verifyLoadTBillsFor() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            ...
            final var sut = mock(MonthlyReturnsCacheStorage.class,
                    withSettings()
                            .useConstructor(null, null, tBillsCacheStorage, null, mock(MonthlyReturnsGenerator.class)));
    ...
    void addCashReturns_checkResult() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            ...
            doCallRealMethod().when(sut).addCashReturns(any(), any(), any());
            //ACT
            final HashMap<Holding, RMonthlyReturns> actual = new HashMap<>();
            sut.addCashReturns(List.of(cashHolding), cad, actual);

            //VERIFY
            Assertions.assertEquals(Map.of(cashHolding, new RMonthlyReturns(cad.name(), HoldingType.CASH, tBills)), actual);
        }
    }
    ...
    void addCashReturns_checkResultWhenInterestRateProvided() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            //SETUP
            final var tBillsCacheStorage = mock(TBillsCacheStorage.class);
            var monthlyReturnsGenerator = mock(MonthlyReturnsGenerator.class);
            final var sut = mock(MonthlyReturnsCacheStorage.class,
                    withSettings()
                            .useConstructor(null, null, tBillsCacheStorage, null, monthlyReturnsGenerator));
            ...
            when(monthlyReturnsGenerator.generateReturns(cashHolding)).thenReturn(generatedReturns);
            ...
            //VERIFY
            Assertions.assertEquals(Map.of(cashHolding, new RMonthlyReturns(cad.name(), HoldingType.CASH, generatedReturns)), actual);
        }
    }
    ...
    void checkCurrency_checkResult2() {
        ...
        //VERIFY
        Assertions.assertEquals(com.fintex.smclient.enumeration.Currency.CAD.name(), actual);
    }
