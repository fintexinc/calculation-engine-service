...
class IncomeForecastCacheStorageTest {
    ...
    void load_verifyFilters() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            ...
            Map<Holding, RIncomeForecast> holdingExposureMap = holdings.stream()
                    .collect(Collectors.toMap(holding -> holding, holding -> mock(RIncomeForecast.class)));
    ...
    void incomeForecastMapper_checkResult() {
        ...
        final Map.Entry<Holding, RIncomeForecast> entry = mock(Map.Entry.class);

        when(entry.getKey()).thenReturn(h);

        final RIncomeForecast rIncomeForecast = mock(RIncomeForecast.class);
    ...
    void incomeForecastMapper_checkResult_4() {
        ...
        when(rIncomeForecast.getPaymentFrequencyType()).thenReturn(PaymentFrequencyType.AT_MATURITY);
        when(holding.getType()).thenReturn(HoldingType.FIXED_INCOME);
