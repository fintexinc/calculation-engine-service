...
class EquityMarketCapitalizationCacheStorageTest {
    ...
    void load_verifyFilters() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            ...
            final EquityMarketCapitalizationCacheStorage m = mock(EquityMarketCapitalizationCacheStorage.class, withSettings()
                    .useConstructor(fdsRepo, capitalizationRepository, capitalizationRepository, capitalizationRepository, stockRepository, cacheStatistic));
    ...
    void mapForStocks_verifyConvertRatingsForStocks() {
        try (var mockedFilterUtils = Mockito.mockStatic(FilterUtils.class)) {
            ...
            final EquityMarketCapitalizationCacheStorage m = mock(EquityMarketCapitalizationCacheStorage.class);
            final List<Holding> holdings = List.of(mock(Holding.class));
            final Map<StockHolding, REquityMarketCapitalizationStock> map = new HashMap<>();
    ...
    void convertRatingsForStocks_verifyGetEquityMarketCapType() {
        ...
        final Map<StockHolding, REquityMarketCapitalizationStock> responseMap = Map.of(s,
                new REquityMarketCapitalizationStock("test"));
        ...
        //VERIFY
        verify(m).getEquityMarketCapType(s, responseMap.get(s).getStyleBox(), warnings);
    }
    ...
    void convertRatingsForStocks_verifyOverrideDefaultValues() {
        try (var mockedMapUtils = Mockito.mockStatic(MapUtils.class)) {
            //SETUP
            final EquityMarketCapitalizationCacheStorage m = mock(EquityMarketCapitalizationCacheStorage.class);
            final REquityMarketCapitalizationStock stock = mock(REquityMarketCapitalizationStock.class);
            final Map<EquityMarketCapType, BigDecimal> DEFAULT_MAP = Map.of(GIANT, ZERO, LARGE, ZERO, MEDIUM, ZERO, SMALL, ZERO, MICRO, ZERO);
            final Map<StockHolding, REquityMarketCapitalizationStock> responseMap = Map.of(mock(StockHolding.class), stock);

            when(stock.getStyleBox()).thenReturn(GIANT.name());
    ...
    void convertRatingsForStocks_checkResult() {
        try (var mockedMapUtils = Mockito.mockStatic(MapUtils.class)) {
            ...
            when(stock.getStyleBox()).thenReturn(null);
            when(m.getEquityMarketCapType(any(), any(), any())).thenReturn(GIANT);
            doCallRealMethod().when(m).convertRatingsForStocks(any(), any());
            //ACT
    ...
    void mapForNonStock_verifyConvertRatings() {
        ...
        final REquityMarketCapitalization r = mock(REquityMarketCapitalization.class);

        final Map<Holding, REquityMarketCapitalization> holdings = Map.of(h, r);
        final Map.Entry<Holding, REquityMarketCapitalization> entry =
    ...
    void convertRatings_verifyMapRatingsToRequiredFormat() {
        ...
        final REquityMarketCapitalization r = new REquityMarketCapitalization(Map.of("test", ONE));
        final Map.Entry<Holding, REquityMarketCapitalization> entry =
