...
class DistributionOfReturnsServiceImplTest {
    ...
    @Test
    void perform_verifyValidateDistributionOfReturns() {
        //SETUP
        final var monthlyrReturnsService = mock(MonthlyReturnsService.class);
        final var requestValidator = mock(DistributionOfReturnsReqValidator.class);
        final var sut = mock(DistributionOfReturnsServiceImpl.class, withSettings()
                .useConstructor(monthlyrReturnsService, requestValidator));

        final var reqDTO = mock(DistributionOfReturnsReqDTO.class);
        final var treeMap = new TreeMap<>(getPortfolioTotalReturns());
        final var calculationDTO = mock(CalculationDTO.class);

        when(reqDTO.getCustomNumberOfBins()).thenReturn(20);
        when(sut.buildCalculationDto(any(), any())).thenReturn(calculationDTO);
        when(calculationDTO.getWeightedAveragePortfolioReturns()).thenReturn(treeMap);

        doCallRealMethod().when(sut).perform(reqDTO);
        //ACT
        sut.perform(reqDTO);
    ...
    void buildCalculationDto_verifyGetPortfolioTotalReturns() {
        ...
        //VERIFY
        verify(monthlyReturnsService).getPortfolioMonthlyReturns(reqDTO.getHoldings(), Currency.CAD, ReturnFactorScale.SCALE_OF_TWO);
    }
    ...
    void buildCalculationDto_verifyGetWeightedAverageWithCpsdAndCpedValidation() {
        ...
        when(monthlyReturnsService.getPortfolioMonthlyReturns(reqDTO.getHoldings(), reqDTO.getCurrency(), ReturnFactorScale.SCALE_OF_TWO)).thenReturn(monthlyReturns);
        ...
        //VERIFY
        verify(monthlyReturnsService).getWeightedAverageWithCpsdAndCpedValidation(monthlyReturns, reqDTO.getCustomPsd(), reqDTO.getCustomPed());
    }
    ...
    void buildCalculationDto_checkResult() {
        ...
        when(monthlyReturnsService.getPortfolioMonthlyReturns(reqDTO.getHoldings(), reqDTO.getCurrency(), ReturnFactorScale.SCALE_OF_TWO)).thenReturn(monthlyReturns);
        when(monthlyReturnsService.getWeightedAverageWithCpsdAndCpedValidation(monthlyReturns, reqDTO.getCustomPsd(), reqDTO.getCustomPed())).thenReturn(portfolioTotalReturns);
