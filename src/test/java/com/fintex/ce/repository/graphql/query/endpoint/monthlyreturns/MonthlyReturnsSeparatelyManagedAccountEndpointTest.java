...
class MonthlyReturnsSeparatelyManagedAccountEndpointTest {
    ...
    void queryDefinition_verify() {
        //SETUP
        final MonthlyReturnsSeparatelyManagedAccountEndpoint sut = mock(MonthlyReturnsSeparatelyManagedAccountEndpoint.class);
    ...
    void responseMapper_verifyMonthlyReturns_FASCurrencyMissing() {
        try (var mockedGraphQlMapperUtils = Mockito.mockStatic(GraphQlMapperUtils.class)) {
            ...
            //VERIFY
            mockedGraphQlMapperUtils.verify(() -> GraphQlMapperUtils.monthlyReturns(monthlyReturns, CurrencyType.USD.name(), smaHolding));
        }
    }
