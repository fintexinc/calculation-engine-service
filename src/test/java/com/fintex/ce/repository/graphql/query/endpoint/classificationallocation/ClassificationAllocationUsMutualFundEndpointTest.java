...
class ClassificationAllocationUsMutualFundEndpointTest {

    @Test
    void getGetUsFundsByTickers_isPresent() {
        //SETUP
        final ClassificationAllocationUsMutualFundEndpoint sut = new ClassificationAllocationUsMutualFundEndpoint();
    ...
    void queryDefinition_verify() {
        //SETUP
        final ClassificationAllocationUsMutualFundEndpoint sut = mock(ClassificationAllocationUsMutualFundEndpoint.class);

        final QueryQuery qq = mock(QueryQuery.class);
        final List<String> tickers = List.of("Ticker");


        doCallRealMethod().when(sut).queryDefinition(any(), any());

        //ACT
        final QueryQueryDefinition actual = sut.queryDefinition(tickers, mock(UnaryOperator.class));
        actual.define(qq);

        //VERIFY
        verify(qq).getUsFundsByTickers(eq(tickers), any());
    }
    ...
    void requestMapper_verify() {
        ...
        when(usFundQuery.securityClassification(any())).thenReturn(usFundQuery);
        when(usFundQuery.securityClassificationAllocation(any())).thenReturn(usFundQuery);
    ...
    void responseMapper_verifyWithSecurityClassificationAllocationData() {
        ...
        when(usFund.getSecurityClassificationAllocation()).thenReturn(securityClassificationAllocation);
        when(usFund.getSecurityClassification()).thenReturn(securityClassification);
        ...
        //ACT
        final RClassificationAllocation result = sut.responseMapper(usFund, usMutualFundHolding);
    ...
    void responseMapper_verifyWithSecurityClassificationData() {
        ...
        when(usFund.getSecurityClassificationAllocation()).thenReturn(null);
        when(usFund.getSecurityClassification()).thenReturn(securityClassification);
    ...
    void responseMapper_verifyWithoutSecurityClassificationData() {
        ...
        when(usFund.getSecurityClassificationAllocation()).thenReturn(null);
        when(usFund.getSecurityClassification()).thenReturn(null);
