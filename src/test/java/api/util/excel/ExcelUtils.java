...
public class ExcelUtils {
    ...
    public static LocalDate readAsLocalDate(Row row, int cellIndex) {
        ...
        return cell.getCachedFormulaResultType().equals(NUMERIC) ? convertToLocalDateViaInstant(cell.getDateCellValue()) : null;
    }
    ...
    public static Map<LocalDate, BigDecimal> readRowData(final Sheet sheet, final int rowNumber,
        ...
        final Map<Integer, java.time.LocalDate> dates = readDates(sheet, dateRowNum, cellToStart);
        final Map<Integer, java.math.BigDecimal> returns = new HashMap<>();
        ...
        for (int i = cellToStart; i < row.getLastCellNum(); i++) {
            ...
            if (cell.getCellType() == CellType.FORMULA && cell.getCachedFormulaResultType() == CellType.NUMERIC) {
                final var returnValue = toUserScale(java.math.BigDecimal.valueOf(cell.getNumericCellValue()));
    ...
    public static LocalDate getLocalDate(final Sheet sheet, final Integer rowNumber, final Integer cellNumber) {
        ...
        return nonNull(cell) && cell.getCachedFormulaResultType() == NUMERIC ? ExcelFormatUtils.convertToLocalDateViaInstant(cell.getDateCellValue()) : null;
    }
