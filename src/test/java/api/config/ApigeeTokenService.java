...
public class ApigeeTokenService {
    ...
    private static String queryOAuthToken() {
        ...
        try {
            final ResponseEntity<OAuth2Token> responseEntity = REST_TEMPLATE.exchange(url, HttpMethod.POST, entity, OAuth2Token.class);
            ...
            throw new SystemException(String.format("Apigee error response status %s and body %s", ex.getStatusText(), ex.getResponseBodyAsString()),
                    ErrorCode.INTERNAL_SERVER_ERROR);
        }
    }

    public static synchronized String getApigeeToken() {
        if (Strings.isNullOrEmpty(System.getProperty(APIGEE_OAUTH2_TOKEN)) || System.currentTimeMillis() > Long.parseLong(System.getProperty(APIGEE_EXPIRES))) {
