...
public class GlobalExceptionHandler {
    ...
    private final ExceptionHandlingService reqErrorHandlingService;

    @Autowired
    public GlobalExceptionHandler(final ExceptionHandlingService reqErrorHandlingService) {
    ...
    public RuntimeExceptionDTO globalExceptionHandler12(final NullPointerException e, final HttpServletRequest request) {
        log.error(e);
        CompletableFuture.runAsync(() -> reqErrorHandlingService.removeRedisCacheForRequestedHoldings(request, e, request.getRequestURI()));
        final HttpStatus status = HttpStatus.INTERNAL_SERVER_ERROR;
        return new RuntimeExceptionDTO(List.of(new ErrorRes2DTO(status.name(), e.getMessage())));
    }

    @ExceptionHandler(value = {DataErrorException.class})
    public ResponseEntity<RuntimeExceptionDTO> globalExceptionHandler12(final DataErrorException e, final HttpServletRequest request) {
        log.error(e.getMessage());
        reqErrorHandlingService.ifFxRatesErrorRemoveFxRatesFromRedisCache(e);
        CompletableFuture.runAsync(() -> reqErrorHandlingService.removeRedisCacheForRequestedHoldings(request, e, request.getRequestURI()));
        return new ResponseEntity<>(new RuntimeExceptionDTO(List.of(new ErrorRes2DTO(e))), e.getHttpStatus());
    }
    ...
    public RuntimeExceptionDTO generalExceptionHandler14(final SystemException e, final HttpServletRequest request) {
        log.error(e);
        final HttpStatus status = e.getErrorCode().getHttpStatus();
        return new RuntimeExceptionDTO(List.of(new ErrorRes2DTO(status.name(), e.getMessage())));
    }

    @ExceptionHandler(ReqValidationException.class)
    @ResponseStatus(value = HttpStatus.BAD_REQUEST)
    public RuntimeExceptionDTO requestValidationExceptionHandler(final ReqValidationException e, final HttpServletRequest request) {
        log.error(e.getMessage());
