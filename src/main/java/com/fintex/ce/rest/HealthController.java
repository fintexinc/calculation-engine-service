    ...
    public ResponseEntity<String> cacheWarmupHealthCheck() {
        try {
            ...
            if (!schedulerRunInfoDto.isRunInLast24Hours()) {
                return new ResponseEntity<>(ERROR_RESPONSE + ", cache warm-up scheduler didn't run in past 24 hours", HttpStatus.INTERNAL_SERVER_ERROR);
            }
            return new ResponseEntity<>(HEALTHY_RESPONSE + ", last time cache warm-up scheduler run: " + schedulerRunInfoDto.getLastTimeRun(), HttpStatus.OK);
