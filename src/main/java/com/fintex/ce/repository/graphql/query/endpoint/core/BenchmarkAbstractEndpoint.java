...
public abstract class BenchmarkAbstractEndpoint<Res extends RedisId>
        extends AbstractSMEndpoint<BenchmarkIndexHolding, String, IndexQuery, Index, Res> {
    ...
    @Override
    public QueryQueryDefinition queryDefinition(final List<String> morningstarIds, final UnaryOperator<IndexQuery> preDefinedSMQuery) {
        return q -> q.getIndexesByMorningstarIds(morningstarIds, (args) -> args.includeNonExistingIndexes(true), preDefinedSMQuery::apply);
    }
    ...
    @Override
    BenchmarkIndexHolding findHoldingBasedOnRes(final List<BenchmarkIndexHolding> holdings, final Index index) {
     ...
     *
     * @param index index object from FDS
     * @return ids
     */
    List<String> getIds(final Index index) {
    ...
    @Override
    protected void populateEmptyResponseWithIdentifier(final List<Index> responses, final BenchmarkIndexHolding holding) {
    ...
    @Override
    protected List<BenchmarkIndexHolding> getNotExistingHoldings(final List<BenchmarkIndexHolding> holdings, final List<Index> responses) {
    ...
    private boolean holdingsThatDontHaveCorrespondingResponseFromFds(final List<List<String>> responseIds, final BenchmarkIndexHolding h) {
        return responseIds.stream().noneMatch(r -> r.contains(h.getMrStarId()));
    }

    @Override
    public Res basicResponseMapper(final Index qEntity, final BenchmarkIndexHolding holding) {
