...
public abstract class SeparatelyManagedAccountAbstractEndpoint<Res extends RedisId>
        extends AbstractSMEndpoint<SmaHolding, SmaIdentifier, SeparatelyManagedAccountQuery, SeparatelyManagedAccount, Res> {

    public SeparatelyManagedAccountAbstractEndpoint(final Function<Query, List<SeparatelyManagedAccount>> getSMEntityFunction,
    ...
    @Override
    SmaHolding findHoldingBasedOnRes(final List<SmaHolding> holdings,
     ...
     *
     * @param separatelyManagedAccount SeparatelyManagedAccount object from FDS
     * @return ids
     */
    private List<String> getIds(final SeparatelyManagedAccount separatelyManagedAccount) {
        final List<ExternalIdentifierTypeValue> codes = Objects.requireNonNull(separatelyManagedAccount.getExternalIdentifiers()).getCodes();
    ...
    @Override
    public Res basicResponseMapper(final SeparatelyManagedAccount separatelyManagedAccount,
                                   final SmaHolding holding) {
        final Res response = responseMapper(separatelyManagedAccount, holding);
        response.setHoldingId(holding.generateUserIdentifier());
        return response;
    }

    @Override
    protected void populateEmptyResponseWithIdentifier(final List<SeparatelyManagedAccount> responses,
                                                       final SmaHolding holding) {
    ...
    @Override
    protected List<SmaHolding> getNotExistingHoldings(final List<SmaHolding> holdings,
     ...
     *
     * @param separatelyManagedAccounts response data from FDS
    ...
    private List<String> getIdentifiers(final List<SeparatelyManagedAccount> separatelyManagedAccounts) {
        return separatelyManagedAccounts.stream()
                ...
                .filter(externalIdentifier -> Objects.equals(externalIdentifier.getType(), ExternalIdentifierType.MORNINGSTAR_ID)
