...
public abstract class FixedIncomeAbstractEndpoint<Res extends RedisId>
        extends AbstractSMEndpoint<FixedIncomeHolding, String, FixedIncomeQuery, FixedIncome, Res> {
    ...
    @Override
    FixedIncomeHolding findHoldingBasedOnRes(final List<FixedIncomeHolding> holdings,
     ...
     *
     * @param fixedIncome FixedIncome object from FDS
     * @return ids
     */
    private List<String> getIds(final FixedIncome fixedIncome) {
        final List<ExternalIdentifierTypeValue> codes = Objects.requireNonNull(fixedIncome.getExternalIdentifiers()).getCodes();
    ...
    @Override
    public Res basicResponseMapper(final FixedIncome fixedIncome,
                                   final FixedIncomeHolding holding) {
        final Res response = responseMapper(fixedIncome, holding);
        response.setHoldingId(holding.generateUserIdentifier());
        return response;
    }

    @Override
    protected void populateEmptyResponseWithIdentifier(final List<FixedIncome> responses,
                                                       final FixedIncomeHolding holding) {
    ...
    @Override
    protected List<FixedIncomeHolding> getNotExistingHoldings(final List<FixedIncomeHolding> holdings,
     ...
     *
     * @param fixedIncomes response data from FDS
    ...
    private String getAdpNumber(final ExternalIdentifiers externalIdentifiers) {
        return externalIdentifiers.getCodes()
                .stream()
                .filter(externalIdentifier -> Objects.equals(externalIdentifier.getType(), ExternalIdentifierType.BROADRIDGE_ADP_NUMBER))
                .map(ExternalIdentifierTypeValue::getValue)
                .findFirst()
                .orElse(null);
    }
