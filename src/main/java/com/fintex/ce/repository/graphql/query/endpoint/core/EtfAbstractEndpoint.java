...
@Log4j2
public abstract class EtfAbstractEndpoint<Res extends RedisId> extends AbstractSMEndpoint<EtfHolding, String, EtfQuery, Etf, Res> {
    ...
    @Override
    public QueryQueryDefinition queryDefinition(final List<String> equityIdentifiers, final UnaryOperator<EtfQuery> preDefinedSMQuery) {
    ...
    @Override
    EtfHolding findHoldingBasedOnRes(final List<EtfHolding> holdings, final Etf etf) {
    ...
    @Override
    public Res basicResponseMapper(final Etf etf, final EtfHolding holding) {
        final Res response = responseMapper(etf, holding);
        response.setHoldingId(holding.generateUserIdentifier());
        return response;
    }

    @Override
    protected void populateEmptyResponseWithIdentifier(final List<Etf> responses, final EtfHolding etfHolding) {
     ...
     */
    protected List<EtfHolding> getNotExistingHoldings(final List<EtfHolding> holdings,
     ...
     */
    protected List<String> getTickersFromResponse(final List<Etf> responses) {
