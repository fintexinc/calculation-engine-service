...
import com.fintex.ce.repository.redis.TBillsRepository;
...
import static com.fintex.ce.config.CacheConfig.LOAD_TBILLS;
...
public class TBillsCacheStorage {
    ...
    public TBillsCacheStorage(final CommonEndpointsComponent commonEndpointsComponent, final TBillsRepository tBillsRepository) {
    ...
    public TreeMap<LocalDate, BigDecimal> loadTBillsFor(final Currency currency) {
        ...
        if (tBills.isEmpty()) {
            final TreeMap<LocalDate, BigDecimal> loadedTBills =
                    (TreeMap<LocalDate, BigDecimal>) commonEndpointsComponent.loadTreasuryBillsBy(mapToFdsCurrency(currency));
    ...
    protected com.fintex.smclient.enumeration.Currency mapToFdsCurrency(Currency currency) {
