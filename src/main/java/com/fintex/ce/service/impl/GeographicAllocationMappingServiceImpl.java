...
import com.fintex.ce.dto.GeographicAllocationDTO;
...
public class GeographicAllocationMappingServiceImpl implements GeographicAllocationMappingService {
     ...
     */
    @Override
    public Map<Holding, Map<GeographicRegionType, BigDecimal>> mapToGeographicRegions(final Map<Holding, Map<String, BigDecimal>> holdingAllocations,
     ...
     */
    private Map<GeographicRegionType, BigDecimal> mapToRegions(final Holding holding, final Map<String, BigDecimal> allocations,
                                                               final List<Warning> warnings, final ExceptionCode errorCode) {
    ...
    private void sumAllocations(final Map<GeographicRegionType, BigDecimal> map, final BigDecimal value, final GeographicRegionType region) {
    ...
    private Map<String, GeographicAllocationDTO> initGeographicAllocationMapping() {
        ...
        if (in == null) {
            final String message = String.format("Geographic Allocation Mapping is missing from path %s", GEOGRAPHIC_ALLOCATION_MAPPING_PATH);
        ...
        return list.stream().filter(e -> e.getRegion() != null).collect(toMap(GeographicAllocationDTO::getCountryId, e -> e));
    }
