...
import static com.fintex.ce.config.enumeration.calculation.ClassificationAllocationType.CASH_AND_CASH_EQUIVALENTS__CANADA;
import static com.fintex.ce.config.enumeration.calculation.ClassificationAllocationType.CASH_AND_CASH_EQUIVALENTS__US;
...
public class ClassificationAllocationCacheStorage
        extends MultipleCacheStorageAbstract<RClassificationAllocation, RClassificationAllocation, RClassificationAllocation, RClassificationAllocation> {
    static final Map<ClassificationAllocationType, BigDecimal> DEFAULT_MAP;
    ...
    public ClassificationAllocationCacheStorage(final ClassificationAllocationSMRepository fdsRepo,
    ...
    @Override
    public Map<Holding, Map<ClassificationAllocationType, BigDecimal>> load(final List<Holding> holdings, final List<DataProvider> providers,
        ...
        map.putAll(getCashTypeByCurrency(filterHoldings(holdings, CASH_PREDICATE), CashHolding::getCurrency, Currency.CAD, CASH_AND_CASH_EQUIVALENTS__CANADA));
        map.putAll(getCashTypeByCurrency(filterHoldings(holdings, CASH_PREDICATE), CashHolding::getCurrency, Currency.USD, CASH_AND_CASH_EQUIVALENTS__US));
        map.putAll(getCashTypeByCurrency(filterHoldings(holdings, GIC_PREDICATE), GicHolding::getCurrency, Currency.CAD, CASH_AND_CASH_EQUIVALENTS__CANADA));
        map.putAll(getCashTypeByCurrency(filterHoldings(holdings, GIC_PREDICATE), GicHolding::getCurrency, Currency.USD, CASH_AND_CASH_EQUIVALENTS__US));
    ...
    public <T extends Holding> Map<Holding, Map<ClassificationAllocationType, BigDecimal>> getCashTypeByCurrency(final List<T> holdings,
    ...
    <H extends Holding> Map<Holding, Map<ClassificationAllocationType, BigDecimal>> mapResponse(final Map<H, RClassificationAllocation> holdings,
                                                                                                final List<Warning> warnings) {
        return holdings.entrySet().stream()
                .collect(toMap(Map.Entry::getKey, e -> getClassificationAllocationMapper(e, warnings)));
    }

    <H extends Holding> Map<ClassificationAllocationType, BigDecimal> getClassificationAllocationMapper(final Map.Entry<H, RClassificationAllocation> entry,
                                                                                                        final List<Warning> warnings) {
        final Map<ClassificationAllocationType, BigDecimal> map = new EnumMap<>(DEFAULT_MAP);
        final RClassificationAllocation entryValue = entry.getValue();
