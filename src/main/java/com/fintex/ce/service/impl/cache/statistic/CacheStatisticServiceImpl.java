...
public class CacheStatisticServiceImpl implements CacheStatisticService {
    ...
    Map<DayOfWeek, Function<Parameters, Integer>> dayOfWeekMap;
    ...
    <H extends Holding, R extends RedisId> void performAnalysis(final Map<H, R> responses,
                                                                final CacheNameEntity cacheNameEntity,
                                                                final CacheCategory cacheCategory) {
     ...
     *
     * @param responses       responses from FDS
     * @param cacheNameEntity cache name prefix
     * @param cacheCategory   cache category
     * @param <H>             holding type
     * @param <R>             response type
     */
    <H extends Holding, R extends RedisId> void incrementForCurrentDay(final Map<H, R> responses,
                                                                       final CacheNameEntity cacheNameEntity,
                                                                       final CacheCategory cacheCategory) {
        responses.forEach((h, r) -> {
            final DayOfWeek dayOfWeek = currentDayOfWeek();
            final Parameters parameters = new Parameters(cacheNameEntity, cacheCategory, h.generateUserIdentifier(), r.getProvider());
     ...
     * @param <R>             response type
     */
    <H extends Holding, R extends RedisId> void createNewRecords(final Map<H, R> responses,
                                                                 final CacheNameEntity cacheNameEntity,
                                                                 final CacheCategory cacheCategory) {
     ...
     */
    <H extends Holding, R extends RedisId> List<SMUsageStatistics> mappedUnsavedHoldings(final Map<H, R> responses,
                                                                                          final CacheNameEntity cacheNameEntity,
                                                                                          final CacheCategory cacheCategory) {
    ...
    SMUsageStatistics mapToStatisticEntity(final Holding h, final RedisId r, final CacheNameEntity cacheNameEntity, final CacheCategory cacheCategory) {
    ...
    Map<DayOfWeek, Function<Parameters, Integer>> initDayOfWeekFunctionMap() {
        Map<DayOfWeek, Function<Parameters, Integer>> map = new EnumMap<>(DayOfWeek.class);
        map.put(SUNDAY, args -> statisticsRepo.incrementSunday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(MONDAY, args -> statisticsRepo.incrementMonday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(TUESDAY, args -> statisticsRepo.incrementTuesday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(WEDNESDAY, args -> statisticsRepo.incrementWednesday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(THURSDAY, args -> statisticsRepo.incrementThursday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(FRIDAY, args -> statisticsRepo.incrementFriday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        map.put(SATURDAY, args -> statisticsRepo.incrementSaturday(args.cacheNameEntity, args.cacheCategory, args.holdingId, args.dataProvider));
        return map;
    }
