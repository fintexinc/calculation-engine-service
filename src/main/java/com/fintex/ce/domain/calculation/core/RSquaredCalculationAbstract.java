 ...
 */
public abstract class RSquaredCalculationAbstract<T extends PeriodResDTO> extends PortfolioBenchmarkCalculationAbstract<T> {
    ...
    public BigDecimal calculateRSquared(final SortedMap<LocalDate, BigDecimal> portfolioExcessReturnByPeriod,
                                        final SortedMap<LocalDate, BigDecimal> benchmarkExcessReturnByPeriod,
                                        final BigDecimal portfolioExcessAverage) {
        final BigDecimal sumSquaredRegression = calculateSumSquaredRegression(portfolioExcessReturnByPeriod, benchmarkExcessReturnByPeriod);
        final BigDecimal totalSumOfSquares = calculateTotalSumOfSquares(portfolioExcessReturnByPeriod, portfolioExcessAverage);
     ...
     */
    BigDecimal calculateSumSquaredRegression(final SortedMap<LocalDate, BigDecimal> portfolioExcessReturnByPeriod,
                                             final SortedMap<LocalDate, BigDecimal> benchmarkExcessReturnByPeriod) {
        return portfolioExcessReturnByPeriod.entrySet().stream().map(e -> (e.getValue().subtract(benchmarkExcessReturnByPeriod.get(e.getKey()))).pow(2))
     ...
     */
    BigDecimal calculateTotalSumOfSquares(final SortedMap<LocalDate, BigDecimal> portfolioExcessReturnByPeriod,
                                          final BigDecimal portfolioExcessAverage) {
        return portfolioExcessReturnByPeriod.values().stream().map(bigDecimal -> (bigDecimal.subtract(portfolioExcessAverage)).pow(2))
                .reduce(ZERO, BigDecimal::add);
    }
}
