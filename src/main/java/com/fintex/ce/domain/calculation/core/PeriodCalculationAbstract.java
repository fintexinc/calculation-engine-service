 ...
 */
@Getter
public abstract class PeriodCalculationAbstract<T extends PeriodResDTO, V> implements PeriodBasedCalculation<T> {

    private static final long ONE_MONTH = 1;

    protected final Set<String> defaultPeriods;
    protected NavigableMap<LocalDate, BigDecimal> portfolioTotalReturns;
     ...
     */
    Pair<String, V> calculateForPeriod(final String period) {
     ...
     */
    boolean isSinceCustomIntervalPerformanceStartDateValid() {
        return cipsd != null
     ...
     *
     * @param resultSet result set (pre-calculated the rest of the periods)
     * @param periods   requested periods
     */
    void addSinceCustomIntervalPerformanceStartDate(final Set<Pair<String, V>> resultSet, final Set<String> periods) {
     ...
     */
    protected V calculatePeriodForCustomIntervalStartDate() {
     ...
     * @param period  user entered period
     * @return number of months
     */
    int getNumberOfMonthsFor(final NavigableMap<LocalDate, BigDecimal> returns, final String period) {
        if (isNumeric(period)) {
     ...
     *
     * @param returns entered returns
     * @return number of months since the beginning of the year
     */
    int getNumberOfMonthsForYearToDate(final Map<LocalDate, BigDecimal> returns) {
     ...
     *
     * @param returns given returns
     * @return number of months
     */
    protected int getNumberOfMonthsForSinceInception(final NavigableMap<LocalDate, BigDecimal> returns) {
     ...
     *
     * @param numberOfMonths number of months
     * @param returns        given map
     * @return product
     */
    public BigDecimal calculateProductForPeriod(final int numberOfMonths, final NavigableMap<LocalDate, BigDecimal> returns) {
    ...
    public List<BigDecimal> getBenchmarkValues(final int numberOfMonths,
        ...
        final NavigableMap<LocalDate, BigDecimal> monthsList = filterRequiredMonthsForPeriod(numberOfMonths, getPortfolioTotalReturns());
     ...
     *
     * @param periods user entered periods
     * @return periods
     */
    Set<String> getInitialPeriods(final Set<String> periods) {
     ...
     *
     * @param responseDTO user defined period based response object
     */
     ...
     *
     * @param result calculated periods (final result)
     ...
     * @param returns        portfolio or benchmark returns
     ...
     *
     * @param periodStartDate date when period starts
     * @param returns         portfolio or benchmark returns
     * @return calculated map
     */
    public SortedMap<LocalDate, BigDecimal> getSubMapByPeriodStartDate(final LocalDate periodStartDate, final NavigableMap<LocalDate, BigDecimal> returns) {
     ...
     */
    public static NavigableMap<LocalDate, BigDecimal> calculateExcessReturn(final SortedMap<LocalDate, BigDecimal> totalReturns,
    ...
    /**
     * Returns mapped values from Set<Pair<String, BigDecimal>> to Set<TimeIntervalResDTO>.
     * <p>
     * TimeIntervalResDTO is the final view of the result.
     *
     * @param result final result.
     * @return result as TimeIntervalResDTO object.
     */
    public Set<TimeIntervalResDTO> formTimeIntervalResDTO(Set<Pair<String, BigDecimal>> result) {
        return result.stream().map(e -> new TimeIntervalResDTO(e.getKey(), e.getValue()))
     ...
     * @return
     */
    public NavigableMap<LocalDate, BigDecimal> overrideTotalReturns(final NavigableMap<LocalDate, BigDecimal> totalReturns) {
        return totalReturns.entrySet()
                .stream().collect(toTreeMap(Map.Entry::getKey, e -> DecimalUtils.divide(e.getValue().multiply(HUNDRED).subtract(HUNDRED), HUNDRED)));
    }
     ...
     *
     * @param returns         risk free rate or portfolio return values
     * @param periodStartDate start date of period
     * @param numberOfMonths  number of month in period
