...
public class AnnualReturnCalculation {
    ...
    public AnnualReturnCalculation(final NavigableMap<LocalDate, BigDecimal> weightedAveragePortfolioReturns, final List<Warning> warnings) {
        this.portfolioTotalReturns = weightedAveragePortfolioReturns;
        this.warnings = warnings;
    }

    public AnnualReturnResDTO<Integer> calculate() {
        ...
        final AnnualReturnResDTO<Integer> responseDTO = new AnnualReturnResDTO<>();
        responseDTO.setAnnualReturns(result.entrySet().stream().map(e -> new KeyValueDTO<>(e.getKey(), e.getValue())).toList());
     ...
     *
     * @param portfolioReturns calculated portfolio returns
     * @param years            set of years
     * @return annual returns map
     */
    TreeMap<Integer, BigDecimal> calculateAnnualReturns(final TreeMap<LocalDate, BigDecimal> portfolioReturns,
        ...
        for (final Integer year : years) {
            ...
            if (subMap.size() < 12) {
                log.warn(String.format("Portfolio Returns are missing a few months between period: %s - %s", startDate, endDate));
                continue;
            }
     ...
     */
    void populateBasicDetails(final AnnualReturnResDTO<Integer> responseDTO,
