...
public class MaxDrawdownCalculation extends PeriodCalculationAbstract<MaxDrawdownResDTO, MaxDrawdownDTO> {
    ...
    @Override
    public MaxDrawdownDTO calculatePeriodForNumberOfMonths(final int numberOfMonths) {
        if (numberOfMonths > getPortfolioTotalReturns().size()) {
            return new MaxDrawdownDTO().setTimeIntervalPeriod(String.valueOf(numberOfMonths));
        }
        ...
        if (maxDrawdownEntry.getValue().compareTo(BigDecimal.ZERO) == 0) {
            return new MaxDrawdownDTO().setValue(BigDecimal.ZERO).setTimeIntervalPeriod(String.valueOf(numberOfMonths));
        }
    ...
    @Override
    public MaxDrawdownResDTO defineResponseType(final Set<Pair<String, MaxDrawdownDTO>> result) {
        final MaxDrawdownResDTO maxDrawdownResDTO = new MaxDrawdownResDTO();
        final List<MaxDrawdownDTO> maxDrawdownDTOS = result.stream()
                .map(v -> ofNullable(v.getValue()).orElse(new MaxDrawdownDTO()).setTimeIntervalPeriod(v.getKey()))
     ...
     *
     * @param growth10KByPeriod growth10K values in period range
     * @return max drawdown values for each month in growthOf10KByPeriod map
     */
    NavigableMap<LocalDate, BigDecimal> calculateMaxDrawdownValues(final NavigableMap<LocalDate, BigDecimal> growth10KByPeriod) {
        final NavigableMap<LocalDate, BigDecimal> maximumDrawdownMap = new TreeMap<>();
        growth10KByPeriod.forEach((key, value) -> {
            final NavigableMap<LocalDate, BigDecimal> subMapFromFirstKeyToCustomDate = getSubMapFromFirstKeyToCustomDate(growth10KByPeriod, key);
            final BigDecimal maxValue = subMapFromFirstKeyToCustomDate.values().stream().max(Comparator.naturalOrder()).orElse(null);
            final BigDecimal maxDrawdownValue = DecimalUtils.divide(value.subtract(maxValue), maxValue);
            maximumDrawdownMap.put(key, maxDrawdownValue.compareTo(BigDecimal.ZERO) <= 0 ? maxDrawdownValue : BigDecimal.ZERO);
        });
        return maximumDrawdownMap;
    }

    /**
     * calculates recovery time value. # of months it takes from trough back to a future month when the portfolio value is equal to
     * or greater than the earlier peak value of the max drawdown.
     *
     * @param maximumDrawdownMap map with max drawdown values
     * @param maxDrawdownEntry   entry with max drawdown month and value
     * @param peak               entry with peak month and value
     * @return # of months
     */
    Integer getRecoveryTimeValue(final NavigableMap<LocalDate, BigDecimal> maximumDrawdownMap,
        ...
        final SortedMap<LocalDate, BigDecimal> mapAfterMaxDrawdown = getSubMapByPeriodStartDate(maxDrawdownEntry.getKey(), maximumDrawdownMap);
        ...
        return Objects.nonNull(recoveryTimeEntry) ? getMonthsBetweenDates(maxDrawdownEntry.getKey(), recoveryTimeEntry.getKey(), firstDayOfMonth()) - 1 : null;
    }
    ...
    Map.Entry<LocalDate, BigDecimal> getPeakValue(final NavigableMap<LocalDate, BigDecimal> maximumDrawdownMap,
                                                  final Map.Entry<LocalDate, BigDecimal> maxDrawdownEntry) {
        final NavigableMap<LocalDate, BigDecimal> valuesBeforeTheMaxDrawdown = getSubMapFromFirstKeyToCustomDate(maximumDrawdownMap, maxDrawdownEntry.getKey());
     ...
     */
    Map.Entry<LocalDate, BigDecimal> getMaxDrawdownValue(final NavigableMap<LocalDate, BigDecimal> maximumDrawdownMap) {
     ...
     *
     * @param map    input map that should be reduced
     * @param toDate custom date
     * @return reduced map
     */
    NavigableMap<LocalDate, BigDecimal> getSubMapFromFirstKeyToCustomDate(final NavigableMap<LocalDate, BigDecimal> map, final LocalDate toDate) {
