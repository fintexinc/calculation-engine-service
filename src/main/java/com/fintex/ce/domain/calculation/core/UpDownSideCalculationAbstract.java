 ...
 */
public abstract class UpDownSideCalculationAbstract<T extends PeriodResDTO> extends BenchmarkWeightedAverageCalculation<T, BigDecimal> {

    NavigableMap<LocalDate, BigDecimal> portfolioDetermination;
    NavigableMap<LocalDate, BigDecimal> benchmarkDetermination;
    ...
    /**
     * Because size of getBenchmarkTotalReturns() and getPortfolioTotalReturns() could be different
     * We handle that with dummy values in returned result of this method.
     * For example when getBenchmarkTotalReturns() consists of 180 entries and getPortfolioTotalReturns() of 100 entries
     * Then result of this method would contain 1..100 correct values and 101-180 incorrect values.
     * That is done by purpose to increase performance instead of calling this method each time for each period.
     * In this method calculatePeriodForNumberOfMonths(), exists validation that would not allow using incorrect values (101-180)
     */
    TreeMap<LocalDate, BigDecimal> getPortfolioDetermination() {
     ...
     */
    NavigableMap<LocalDate, BigDecimal> getBenchmarkDetermination(final NavigableMap<LocalDate, BigDecimal> portfolioDownsideDetermination) {
    ...
    BigDecimal calculateDeviationFor(int numberOfMonths, final NavigableMap<LocalDate, BigDecimal> determinations) {
