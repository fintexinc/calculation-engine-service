...
public abstract class RollingAbstractCalculation<T extends PeriodResDTO> extends PeriodCalculationAbstract<T, NavigableMap<LocalDate, BigDecimal>> {
     ...
     * <p>
     * Should to be overridden in all Rolling calculations:
     * (Rolling Total Returns, Rolling Standard Deviation, Rolling Sharpe Ratio, Rolling Correlation)
     * and implement its custom logic.
     *
     * @param numberOfMonths period to be calculated.
     * @param returns        portfolio total returns.
    ...
    /**
     * Checks if the portfolio entry if in needed range. It should be greater than startDateOfRollingReturn and
     * less than portfolio End Date.
     *
     * @param returnEntry              portfolio entry.
     * @param startDateOfRollingReturn start date of rollin returns.
     * @param ped                      portfolio performance end date.
     * @return true - if in range, otherwise false.
     */
    boolean isInRange(final Map.Entry<LocalDate, BigDecimal> returnEntry,
    ...
    public Set<RollingIntervalResDTO> getRollingIntervalResDTOS(final Set<Pair<String, NavigableMap<LocalDate, BigDecimal>>> result) {
        return result
                .stream()
                .map(e -> new RollingIntervalResDTO(e.getKey(), mapRollingReturn(e)))
                .collect(Collectors.toSet());
    }

    public Set<IntervalResDTO> mapRollingReturn(final Pair<String, NavigableMap<LocalDate, BigDecimal>> result) {
        if (Objects.isNull(result.getValue())) {
            return null;
        }
        return result.getValue().entrySet().stream()
                .map(e -> new IntervalResDTO(e.getKey(), e.getValue()))
