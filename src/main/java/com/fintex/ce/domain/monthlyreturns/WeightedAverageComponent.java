...
import com.fintex.ce.domain.calculation.formula.SumProduct;
...
public class WeightedAverageComponent {
    ...
    public NavigableMap<LocalDate, BigDecimal> calculateWeightedAverage(final Map<Holding, TreeMap<LocalDate, BigDecimal>> returns) {
    ...
    Map<LocalDate, BigDecimal> calculateTotalPortfolioReturnFactor(final Map<Holding, TreeMap<LocalDate, BigDecimal>> portfolioBaseTotalReturn,
                                                                   final Map<Holding, TreeMap<LocalDate, BigDecimal>> endingPortfolioWeight) {
        final NavigableMap<LocalDate, BigDecimal> calculate = new SumProduct<>(portfolioBaseTotalReturn, endingPortfolioWeight)
    ...
    Map<Holding, TreeMap<LocalDate, BigDecimal>> calculateEndingPortfolioWeight(final Map<Holding, TreeMap<LocalDate, BigDecimal>> pBaseTotalReturn) {
        final Map<Holding, BigDecimal> initialWeights = PortfolioUtils.calculateInitialPortfolioWeight(pBaseTotalReturn.keySet());
        return pBaseTotalReturn.entrySet().stream().collect(toMap(Map.Entry::getKey, collectMonthlyWeightEntries(initialWeights)));
    }

    /**
     * Works for monthly rebalance
     *
     * @param initialWeights initial holdings weights of portfolio
     * @return function to collect starting portfolio holdings weights for each time period for single holding
     */
    Function<Map.Entry<Holding, TreeMap<LocalDate, BigDecimal>>, TreeMap<LocalDate, BigDecimal>> collectMonthlyWeightEntries(
        ...
        return topEntry -> {
            ...
            return hBaseTotalReturns.entrySet().stream().collect(toTreeMap(Map.Entry::getKey, e -> initialWeights.get(topEntry.getKey())));
        };
    }
}
