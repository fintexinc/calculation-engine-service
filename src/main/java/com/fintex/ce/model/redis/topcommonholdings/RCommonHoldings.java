...
public class RCommonHoldings extends RedisId implements Serializable {
    ...
    /**
     * The data in this Redis Object will be saved as String. And when we want to work with
     * the List of CommonHoldingsDTO then we just deserialize to "List<CommonHoldingsDTO>".
     * <p>
     * The reason of that is when the data is as "List<CommonHoldingsDTO>" then if a list has more than 30K objects
     * then the response time is more then 4 minutes, which is not OK.
     * The data saved as String returns in less than 1 second.
     *
     * deserializedHoldings is used for Caffeine cache.
     * Redis doesn't store deserializedHoldings
     *
