...
public class TopCommonHoldingsReqValidation extends ReqValidation {
    ...
    @Override
    public void check() {
        final int sizeOfAccumulateHoldingTypes = CollectionUtils.isEmpty(reqDTO.getAccumulateHoldingTypes()) ? 0 : reqDTO.getAccumulateHoldingTypes().size();
    ...
    private boolean checkGicHoldingName(List<Holding> holdings){
        ...
        final long holdingsWithoutName = gicHoldings.stream().filter(h -> Objects.isNull(h.getName()) || h.getName().isEmpty()).count();
