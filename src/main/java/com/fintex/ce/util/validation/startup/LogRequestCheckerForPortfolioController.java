...
import com.fintex.ce.rest.PortfolioController;
        ...
        for (Annotation annotation : annotations) {
            if (annotation instanceof LogRequest) {
                for (final var method : PortfolioController.class.getDeclaredMethods()) {
                    if (Arrays.stream(method.getDeclaredAnnotations()).map(Annotation::annotationType).collect(Collectors.toList()).contains(PostMapping.class)) {
                ...
                if (!incorrectMethods.isEmpty()) {
                    throw new GeneralRuntimeException("Following methods in PortfolioController must have HttpServletRequest as parameter" +
