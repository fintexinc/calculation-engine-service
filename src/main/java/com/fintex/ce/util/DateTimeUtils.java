...
public class DateTimeUtils {
    ...
    public static int getMonthsBetweenDates(final LocalDate startDate, final LocalDate endDate, final TemporalAdjuster temporalAdjuster) {
    ...
    public static List<LocalDate> rangeWithLastDayOfMonth(final LocalDate startDate, final LocalDate endDate) {
        ...
        final int monthsBetween = Math.max(1, (int) ChronoUnit.MONTHS.between(startFrom.withDayOfMonth(1), endDate.withDayOfMonth(1)));
    ...
    //Value of startDate is already verified in HoldingReqValidation::validateGicHolding and can't be quincentenary before now
    ...
    private static LocalDate limitStartDate(final LocalDate startDate, final LocalDate endDate) {
        ...
        if (startDate.isBefore(quincentenaryBeforeEnd)) {
            log.warn("Start date {} is more than quincentenary before end date {}. Setting start date to quincentenary before end date {}.",
