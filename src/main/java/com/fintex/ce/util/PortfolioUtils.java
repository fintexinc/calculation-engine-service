...
public class PortfolioUtils {
    ...
    public static Map<Holding, Map<LocalDate, BigDecimal>> fxRatesForHoldings(
        ...
        return holdings.entrySet().stream().collect(toMap(Map.Entry::getKey, entry -> fxRatesForHolding(fxRates, entry.getValue(), toCurrency)));
    }

    public static Map<LocalDate, BigDecimal> fxRatesForHolding(final Map<LocalDate, FxRatesDTO> fxRates, final Currency from, final Currency to) {
        return fxRates.entrySet().stream().collect(CollectorUtils.toTreeMap(Map.Entry::getKey, mapFxRateBasedOnCurrency(from, to)));
    }

    private static Function<Map.Entry<LocalDate, FxRatesDTO>, BigDecimal> mapFxRateBasedOnCurrency(final Currency from, final Currency to) {
        return entry -> {
            ...
            } else if (Currency.CAD.equals(from) && Currency.CAD.equals(to) || Currency.USD.equals(from) && Currency.USD.equals(to)) {
                return ONE;
            }
     ...
     *
     * @param map contains Map with values for each Holding type
     * @param <T> generic key
     * @return
     */
