...
class FixedIncomeSectorAllocationMapperTest {
  ...
  void shouldFilterOutEntriesWithNullTypeOrValue() {
    ...
    FixedIncomeBondSector result = mapper.map(micResponse, holdingWithoutCountry(new SecurityIdentifier("TEST.ID",
  ...
  void shouldMapCurrency_whenResponseHasCurrency() {
    ...
    FixedIncomeBondSector result = mapper.map(micResponse, holdingWithoutCountry(new SecurityIdentifier("AGG", null),
