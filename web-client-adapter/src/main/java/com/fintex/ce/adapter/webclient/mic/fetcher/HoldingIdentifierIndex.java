...
import com.fintex.ce.adapter.webclient.mic.mapper.HoldingMappingUtils;
 ...
 * financial instrument type into {@link TypedIdentifiers} and keeps a reverse identifier-to-holdings index (a list,
 * because multiple holdings can share the same security, e.g. the same stock in different accounts) used to map
 * {@link SecurityAttributeResult} responses back onto the original holdings. Holdings without an identifier or with a
 * locally-sourced type (see {@link HoldingMappingUtils#isSkipped}) are excluded from Market Investment Catalogue
 * requests.
 */
@Slf4j
