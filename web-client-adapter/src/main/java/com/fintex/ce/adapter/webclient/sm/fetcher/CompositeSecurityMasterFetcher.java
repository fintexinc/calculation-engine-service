...
/**
 * Generic Security Master client that fetches any combination of composite security attributes in a single request to
 * the SMS {@code /attributes} endpoint. The attribute payloads arrive as raw JSON and are converted into the CE domain
 * models declared by the {@link CompositeAttributeBinding} registered for each attribute. Attributes without a
 * registered binding are rejected up front so a misconfigured request fails loudly instead of silently dropping data.
 */
@Slf4j
public class CompositeSecurityMasterFetcher implements SecurityAttributesFetcher {
  ...
  public CompositeSecurityMasterFetcher(SecurityMasterWebClient client, String endpointPath,
