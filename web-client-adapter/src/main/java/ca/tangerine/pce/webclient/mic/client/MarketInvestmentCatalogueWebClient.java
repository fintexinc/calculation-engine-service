...
/**
 * Generic WebClient for Market Investment Catalogue API calls. Provides generic HTTP methods that can be used by
 * specific fetchers. Every call runs through {@link ExternalCallResilience}: transient failures — transport errors,
 * timeouts, 5xx, 408 and 429 — are retried and counted against the circuit breaker, while any other 4xx fails straight
 * through as this service's own bug. Failures of the whole logical call are then split by cause:
 * <ul>
 ...
 *
 * <p>
 * Retries apply to POST as well as GET. That is safe here only because Market Investment Catalogue's POST endpoints are
 * read-only queries — POST carries a request body too large for a URL, it never mutates state. An endpoint that does
 * mutate state must not be called through this client without revisiting that assumption, since a retried mutation is a
 * duplicate one.
 *
 * <p>
...
public class MarketInvestmentCatalogueWebClient {

  private static final String GET = HttpMethod.GET.name();
  private static final String POST = HttpMethod.POST.name();

  private final WebClient micWebClient;
  private final ExternalCallResilience marketInvestmentCatalogueCallResilience;
  ...
  public <T, R> R post(String path, T request, ParameterizedTypeReference<R> responseType) {
    return observe(POST, path, call -> {
      ...
      R result = execute(path, call, micWebClient.post()
          .uri(path)
          .bodyValue(request)
          .retrieve()
  ...
  public <R> R get(String path, Map<String, ?> queryParams, Class<R> responseType) {
    return observe(GET, path, call -> {
      ...
      R result = execute(path, call, micWebClient.get()
  ...
  private <R> R execute(String path, ExternalCall call, Mono<R> request) {
    return marketInvestmentCatalogueCallResilience.decorate(request)
        .onErrorMap(error -> ExternalCallErrorMapper.toDomainError(ExternalWebService.MARKET_INVESTMENT_CATALOGUE, path,
            error,
            call))
        .block();
  }
  ...
  private <R> R observe(String httpMethod, String path, Function<ExternalCall, R> action) {
    ExternalCall call = observability.start(ExternalWebService.MARKET_INVESTMENT_CATALOGUE, httpMethod, path);
    try {
