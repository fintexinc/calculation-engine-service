...
@Slf4j
@Configuration
@EnableConfigurationProperties(MarketInvestmentCatalogueRestProperties.class)
@ConditionalOnProperty(name = "external-services.market-investment-catalogue.api-type", havingValue = "rest", matchIfMissing = true)
public class MarketInvestmentCatalogueWebClientConfig {

  static final String REQUEST_ID_HEADER = "X-Request-ID";
  static final String REQUEST_ID_MDC_KEY = "requestId";

  @Bean
  public WebClient micWebClient(
      WebClient.Builder webClientBuilder,
      MarketInvestmentCatalogueRestProperties properties) {
  ...
  /**
   * Forwards the inbound {@code requestId} to Market Investment Catalogue so both services log the same identifier. The
   * W3C {@code traceparent} header is propagated separately by the Micrometer/OpenTelemetry instrumentation of the
  ...
  private ExchangeFilterFunction logRequest() {
    return ExchangeFilterFunction.ofRequestProcessor(request -> {
      log.debug(">>> MIC Request: {} {}", request.method(), request.url());
      return Mono.just(request);
    });
  }
  ...
  private ExchangeFilterFunction logResponse() {
    return ExchangeFilterFunction.ofResponseProcessor(response -> {
      log.debug("<<< MIC Response: {} - Status: {}", response.request().getURI(), response.statusCode());
