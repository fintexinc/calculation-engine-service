...
class EquityMarketCapitalizationUsMutualFundEndpointTest {
  ...
  void responseMapper_checkResult() {
    try (var mockedGraphQlMapperUtils = Mockito.mockStatic(GraphQlMapperUtils.class)) {
      // SETUP
      final EquityMarketCapitalizationUsMutualFundEndpoint sut = mock(
          EquityMarketCapitalizationUsMutualFundEndpoint.class);
      ...
      doCallRealMethod().when(sut).responseMapper(any(), any());
      // ACT
      final com.fintex.ce.domain.model.EquityMarketCapitalization expected = sut.responseMapper(usFund, h);
