...
class BusinessCountryGraphqlDataFetcherTest {

  @Test
  void queryBenchOfStock_verifyDoQuery() {
    // SETUP
    final var graphqlTransport = mock(GraphqlTransportComponent.class);
    final BusinessCountryGraphqlDataFetcher m = mock(BusinessCountryGraphqlDataFetcher.class, withSettings().useConstructor(
        graphqlTransport));
  ...
  void queryBenchOfStock_checkResult() {
    // SETUP
    final BusinessCountryGraphqlDataFetcher m = mock(BusinessCountryGraphqlDataFetcher.class);
