...
public class AverageMerGraphqlDataFetcher
    extends AbstractGraphqlDataFetcher<AverageMer> {

  public AverageMerGraphqlDataFetcher(GraphqlTransportComponent graphqlTransport) {
    super(graphqlTransport);
  }

  @Override
  protected AbstractSMEndpoint<?, ?, ?, ?, AverageMer> resolveEndpoint(HoldingType type) {
    return switch (type) {
      case CANADA_HEDGE_FUNDS -> new AverageMERCanadaHedgeFundEndpoint();
      case CANADA_ETF -> new AverageMEREtfCanadaEndpoint();
      case US_ETF -> new AverageMEREtfUsEndpoint();
      case CANADA_MUTUAL_FUNDS, SEGREGATED_FUND_CANADA -> new AverageMERFundCanadaEndpoint();
      case US_MUTUAL_FUNDS -> new AverageMERUsMutualFundEndpoint();
      default -> null;
    };
  }
