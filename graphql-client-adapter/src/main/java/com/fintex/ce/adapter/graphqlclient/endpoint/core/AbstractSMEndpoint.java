 ...
 *          id type to query SM
 * @param <QReq>
 *          query request for SM
 * @param <QEntity>
 *          core entity from the SM response
 * @param <Res>
 *          query response from SM
 */
@Slf4j
public abstract class AbstractSMEndpoint<H, QId, QReq, QEntity, Res> {
   ...
   * @param ids
   *          mapped holding ids
   * @param preDefinedSMQuery
   *          pre-defined query for SM
   * @return full SM query
   */
  public abstract QueryQueryDefinition queryDefinition(final List<QId> ids,
      final UnaryOperator<QReq> preDefinedSMQuery);
   ...
   * @param responses
   *          response data from SM
   * @return responses with identifiers
   */
  public List<QEntity> populateIdentifiersIfEmpty(final List<H> holdings, final List<QEntity> responses) {
  ...
  /**
   * checks if there are empty identifiers in response, if yes populates with not existing identifiers (taken from
   * request)
   *
   * @param responses
   *          response data from SM
   * @param holdingsThatDontHaveResponsesFromSm
   *          tickers that don't exist
   */
  public void populateEmptyResponsesWithIdentifiers(final List<H> holdingsThatDontHaveResponsesFromSm,
   ...
   *          holdings from request payload
   * @param responses
   *          all not null tickers from SM response
  ...
  public List<com.fintex.smclient.graphql.DataProvider> loadProviders() {
    ...
    if (filteredProviders.isEmpty()) {
      throw new ReqValidationException(String.format("Provider(-s) %s is not supported by %s", userEnteredProviders,
          this.endpointName));
    }
    return filteredProviders;
  }

  public AbstractSMEndpoint<H, QId, QReq, QEntity, Res> setUserEnteredProviders(
      final List<DataProvider> userEnteredProviders) {
   ...
   *           if ${this.supportedProviders} field is empty
   */
  ...
  List<com.fintex.smclient.graphql.DataProvider> filterDataProviders() {
    ...
    for (DataProvider provider : this.userEnteredProviders) {
      if (this.supportedProviders.contains(provider)) {
        final com.fintex.smclient.graphql.DataProvider smProvider = com.fintex.smclient.graphql.DataProvider.valueOf(
            provider.name());
